...
%left RPAREN LPAREN DASH rwDEFINE rwACTION rwPARAMS rwEFFECT rwPRECOND rwPREDICATES rwREQUIREMENTS rwTYPES rwOBJECTS rwINIT rwGOAL rwDOMAIN rwTYPING rwAND rwOR rwONEOF rwWHEN rwNOT rwIMPLY rwFORALL rwPROBLEM EQUA rwEXISTS rwLENGTH rwCONSTANTS rwEITHER rwINCREASE rwMETRIC rwMINIMIZE
...
%type <atomlistp> atomlist one_of_combine initCollect one_of_structure complex_structure or_combine or_structure not_combine not_structure
%type <i> varid
%type <i> numexpr
...
/* ThanhNH */
one_of_combine : LPAREN rwONEOF atomlist RPAREN {save_one_of($3);}
   ;

or_combine : LPAREN rwOR atomlist RPAREN {save_or($3);}
   ;
   
not_combine : LPAREN rwNOT atomlist RPAREN {save_not($3);}
   ;   
/* End ThanhNH */	
...
def	: rwDOMAIN ID { checkdomain($2); }
	...
	| rwINIT initCollect { storeinit($2);getInitState($2);}
...
/* ThanhNH */	
initCollect: atomlist {$$ = $1;}
    | complex_structure {$$ = $1;}
    | /*empty */
;

complex_structure : one_of_structure {$$ = $1;}
    | or_structure {$$ = $1;}
    | /* Empty */
   
;
or_structure : atomlist or_combine atomlist {$$ = atomlist_combine($3,$1);}
    | atomlist or_combine or_structure {$$ = atomlist_combine($3,$1);}
    | atomlist one_of_combine or_structure {$$ = atomlist_combine($3,$1);}
    | atomlist not_combine or_structure {$$ = atomlist_combine($3,$1);}
    | /* Empty */
;

not_structure : atomlist not_combine atomlist {$$ = atomlist_combine($3,$1);}
    | atomlist not_combine not_structure {$$ = atomlist_combine($3,$1);}
    | atomlist one_of_combine not_structure {$$ = atomlist_combine($3,$1);}
    | atomlist or_combine not_structure {$$ = atomlist_combine($3,$1);}
    | /* Empty */
;

one_of_structure : atomlist one_of_combine atomlist {$$ = atomlist_combine($3,$1);}
    | atomlist one_of_combine one_of_structure {$$ = atomlist_combine($3,$1);}
    | atomlist or_combine one_of_structure {$$ = atomlist_combine($3,$1);}
    | atomlist not_combine one_of_structure {$$ = atomlist_combine($3,$1);}
    | /* Empty */
;
/* End ThanhNH */	
