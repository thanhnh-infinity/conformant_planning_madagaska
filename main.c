...
void givememorystatistics() {
	...
	if (pf) {
		...
		fscanf(pf, "%u %u %u %u %u %u", &size, &resident, &share, &text, &lib,
				&data);
		...
		/*
		 ...
		 data/stack   : %u\n",
		 */
		(MB < 700.0) ? MB : MB / 1024.0, (MB < 700.0) ? "MB" : "GB");
	}
	fclose(pf);
}
...
#include "zPRINTDATE"
	...
	if (argc == 1) {
		printf("\n");
		printf(
				"The command line parameters are the input file names and options:\n");
		printf("-A n   Run algorithm A with parameter n (range 1 to 50)");
		if (evalAlgorithm == 0)
			printf(" default -A %i.\n", paramA);
		else
			printf("\n");
		printf("-B r   Run algorithm B with parameter r (range 0.1 to 0.9999)");
		if (evalAlgorithm == 1)
			printf(" default -B %.2f.\n", paramB);
		else
			printf("\n");
		printf("-C r   Run algorithm C with parameter r (range 1.2 to 2.0)");
		if (evalAlgorithm == 2)
			printf(" default -C %.2f.\n", paramC);
		else
			printf("\n");
		printf(
				"-M n   With algorithm B, use maximum n processes (default -M %i).\n",
				paramM);
		printf(
				"-S n   Step for horizon lengths 0,n,2n,3n,... (default -S %i, algorithms A and B only)\n",
				outputTimeStep);
		...
		printf(
				"-O     Write formulas in a file in DIMACS format instead of solving.\n");
...
#define noEXPLAINSETTINGS 1
	...
	if (planFrontend != 1) {
		...
		if (debugOutput >= 1)
			printf("Preparing to compute invariants...\n");
		...
		sclass = goalclass();
		for (i = 0; i < nOfActions; i++) {
			if (sclass == STRIPS && actionclass(i) == Conjunctive)
				sclass = Conjunctive;
		...
		switch (sclass) {
		case STRIPS:
			printf("Actions: STRIPS\n");
			break;
		case Conjunctive:
			printf("Actions: conjunctive\n");
			break;
		case GeneralPDDL:
			printf("Actions: general\n");
			break;
		}
		...
		findoccurrences();
		if (debugOutput > 0)
			printf("time %.2f for preprocessing for disabling graph\n",
		...
		if (flagOutputDIMACS == 0) {
			...
			do {
				printf("%i %i %i %i\n", seqs[i].sati->nOfTPoints - 1,
						seqs[i].sati->value, seqs[i].sati->conflicts,
